...
当前版本为 **0.10.0：PDF 智能取文、多语言文本整理与实验性日语注音**。保留长 PDF 增量保存、页级断点续识、自然排序、表格和离线模型迁移；不改变 Paddle 模型或 Debian 10 / glibc 2.28 兼容基线。

### 0.10.0 使用方法

打开文档预览上方的“文档设置”：默认 PDF 自动取文、智能整理和跨页正文连接，日语注音识别默认关闭。文字来源和注音识别影响下一次识别；断行、跨页连接、注音输出格式只重新整理已有结果，不会重新 OCR。完整说明和已知限制见 [文本处理说明](docs/TEXT_PROCESSING.md)。
...
0.9.2 基线通过了 80 项 Python 测试和 37 项前端测试；0.10.0 增加真实 PDF 分支、Ruby、整理、导出与交互测试。Windows/Linux 冻结 sidecar、真实模型推理、AppImage 与 NSIS 安装包仍由 GitHub Actions 和目标电脑完成最终验证。
