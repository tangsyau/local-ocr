...
当前版本为 **0.13.1：任务状态、结果导出和界面完善**。Windows x64、Linux x64 AppImage、WebKitGTK 4.0 / glibc 2.28 兼容 AppImage 保留原有构建任务与模型组合。

### 0.13.1 使用方法

- **页码**：全部页 / 指定页码为单选项；输入框常显，输入后自动选择指定页码。离开输入框时校验，直接点开始也会等待校验完成。错误输入不会静默沿用旧范围。“应用到勾选的 PDF”仍用于批量设置，范围不合法时本次批量设置整体不提交。
- **暂停**：等待当前页完成期间显示“正在暂停”；收到后台确认后才能继续。普通取消和跳过也在当前页结束后生效。模型准备期间可单独停止准备，完整缓存保留。
- **PDF 自动取文**：纯文本 PDF 不用事先下载或载入 OCR 模型；遇到扫描页才准备模型，成功后从已完成页面继续。仅本地模型模式下不补下载，已提取的页面仍保留。文本层质量判断属于保守规则，不能保证发现所有错误，异常时请用强制 OCR。
- **设置范围**：文字来源、注音识别作用于队列下一次识别；文本整理、跨页正文、注音输出作用于所有已有文字结果，保留手动校对。页码和图片旋转只作用于当前文件，批量应用按钮另行操作。结果区显示原结果的模式、档位、页码、注音和页面来源。
- **查看与导出分开**：结果区“整理前 / 整理后”只切换预览，不改变导出内容，也不会把查看行为标成内容修改。导出区明确选择“全部已有结果 / 当前文件 / 勾选任务”、整理前 / 整理后及 TXT、XLSX、HTML 组合；默认导出整理后，包含手动校对。导出确认显示实际范围与版本。
- **图片表格批量导出**：只包含导出范围内的图片，未勾选图片不混入，也不会跨过排除项自动合并。无表格不生成 XLSX，部分完成结果可导出。
- **边识别边校对**：校对内容作为已完成页面的独立前缀保留，新到达页面继续自动整理；再次编辑会吸收当时已显示的所有页。已校对部分与后续新页之间保留段落边界，不自动改写用户文字。自动保存保留该边界和逐页数据。
- **专注模式**：文字按整理前后双栏显示，只保留各栏自己的复制按钮。整理后支持真正的 Ruby 预览；关闭“显示日语注音（Ruby）”后可编辑普通文本。显示保存状态，复制结果就近反馈。表格仍采用单栏。
- **长表格**：横向滚动条放在表格阅读区外，滚到中间或底部仍然可用；多表时随当前可见表切换，不遮挡单元格。
- **界面**：常用控件统一 13px，次要说明 12px；提高说明文字对比度和小按钮点击区域。文档设置保持常显，PDF 不再保留大的空白预览占位。未添加任务时简化尚不可用的导出设置；增加任务后自动显示。
- **长 PDF**：自动保存只监听任务摘要，不再深度遍历所有页；共用当前整理投影，减少专注模式的重复计算。基准见 [性能记录](docs/PERFORMANCE-0.13.1.json)，不代表原生安装版或 OCR 模型测速。

日语注音继续支持横排、竖排的正文 / 括号 / HTML Ruby 三种输出。识别与绑定仍可能漏检、错配；手工校对部分不重建 Ruby 对应关系。中日英标点、空格、空行与断行规则保留 0.12.1 修复，完整规则见 [文本处理说明](docs/TEXT_PROCESSING.md)。

原有 0.12.x 会话可继续读取；含新式流式校对的会话如需回退旧程序，应先导出完整结果。模型不需因为本次界面与状态更新而全部重下。

检查范围、验收步骤与未验证项见 [0.13.1 验证记录](docs/VALIDATION-0.13.1.md)。
...
0.13.1 验证记录见 [验证说明](docs/VALIDATION-0.13.1.md)。Windows/Linux 冻结 sidecar、真实模型推理、AppImage 与 NSIS 安装包仍由 GitHub Actions 和目标电脑完成最终验证。
