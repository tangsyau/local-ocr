> 当前修复版：**0.14.3**。修复文本清洗对旧内核 API 的依赖，并增加实际安装包内核检查；详见 [验证记录](docs/VALIDATION-0.14.3.md)。

# Local OCR

## 0.14.2：识别历史与自动导出

- **批量任务 → 识别历史**：搜索文件名、分页浏览，点击“打开结果”恢复到队列并查看、复制或手动导出。不要求原始图片/PDF 仍存在。恢复会采用记录的文本整理、注音及合并设置；这些是当前工作区设置。
- **清理已结束 / 移除**：只清理队列。结果先成功保存才移除；失败则保留任务。历史默认不会自动清理。
- **永久删除**：先移出队列，再在历史中勾选并确认永久删除。会显示未导出数量；不删除原始图片/PDF。
- **自动导出**：开始前选择 TXT / XLSX / HTML 组合，勾选“每个文件完成后自动导出”并选择目录。程序记住配置。每个完整文件完成后先保存历史，再导出；暂停或取消的部分结果仅保留，仍可手动导出。
- 自动导出按文件生成，采用所选文本版本和前后缀，重名自动编号、不覆盖。PDF 内可合并跨页表格；需要将多张图片合成一张表时，使用原有的手动导出。
- 任务分别显示识别状态与导出状态。失败可修改目录或格式，再点“重试导出”；不会重新识别。新校对内容需再次导出。
- 历史保存在本机应用数据中；清除应用数据会丢失历史。重要结果仍建议导出到自己的文件夹。这次升级可保留旧队列的现有结果，无法恢复旧版已经永久删除的数据。旧版程序不认识历史保留机制，不建议降级使用同一数据目录。
...
当前版本为 **0.14.2：任务状态、结果导出和界面完善**。Windows x64、Linux x64 AppImage、WebKitGTK 4.0 / glibc 2.28 兼容 AppImage 保留原有构建任务与模型组合。
...
- **长 PDF**：自动保存只监听任务摘要，不再深度遍历所有页；共用当前整理投影，减少专注模式的重复计算。基准见 [性能记录](docs/PERFORMANCE-0.14.2.json)，不代表原生安装版或 OCR 模型测速。
...
检查范围、验收步骤与未验证项见 [0.14.2 验证记录](docs/VALIDATION-0.14.2.md)。
...
0.14.2 验证记录见 [验证说明](docs/VALIDATION-0.14.2.md)。Windows/Linux 冻结 sidecar、真实模型推理、AppImage 与 NSIS 安装包仍由 GitHub Actions 和目标电脑完成最终验证。
...
- 有未导出结果或正在处理的任务时关闭窗口会提醒。多窗口同时保存发生冲突时会停止覆盖并提示先导出。清理或移除任务只移除队列引用，识别历史继续保留；永久删除需在“识别历史”中单独确认。
...
自动保存的识别内容未加密，属于本机用户数据，并不是诊断日志。使用共享电脑或处理敏感材料后，如需清除识别内容，请先移除队列中的相关任务，再在“识别历史”中永久删除；如需更高保障，应配合操作系统账户隔离及磁盘加密。不要把本机 WebView 数据目录放入云同步目录。
