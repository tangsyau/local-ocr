...
当前版本为 **0.14.1：任务状态、结果导出和界面完善**。Windows x64、Linux x64 AppImage、WebKitGTK 4.0 / glibc 2.28 兼容 AppImage 保留原有构建任务与模型组合。
...
- **长 PDF**：自动保存只监听任务摘要，不再深度遍历所有页；共用当前整理投影，减少专注模式的重复计算。基准见 [性能记录](docs/PERFORMANCE-0.14.1.json)，不代表原生安装版或 OCR 模型测速。
...
检查范围、验收步骤与未验证项见 [0.14.1 验证记录](docs/VALIDATION-0.14.1.md)。
...
0.14.1 验证记录见 [验证说明](docs/VALIDATION-0.14.1.md)。Windows/Linux 冻结 sidecar、真实模型推理、AppImage 与 NSIS 安装包仍由 GitHub Actions 和目标电脑完成最终验证。
