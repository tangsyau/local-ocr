...
当前版本为 **0.9.0：长文档性能与断点续识版**。保留 0.8.x 的指定页码、图片旋转、离线模型迁移和 Windows 初始化修复；仍不增加 PDF 预览、原图联动校对、GPU 或 macOS 构建。

### 0.9.0 优化与升级

- 长 PDF 逐页结果采用增量累计；sidecar 流式识别不再在内存中保留并于结束时重复回传全部页面。
- 自动保存将任务摘要和逐页结果分开存入 IndexedDB，只写入新页面；0.8.x 记录会在读取后自动迁移。
- 失败、取消或异常退出的 PDF 可继续尚未完成的物理页面。只有原文件、页码范围、模型、模式、最低置信度和旋转设置均未变化时才会续接，否则按完整重识别处理并保留原有确认提示。
- 队列增加“按文件名排序”，采用自然数字顺序，适合按页编号的扫描图片。
- 修复重新识别在第一页前失败时旧结果的模式标记未同步恢复的问题。
- 如用标签发布，请使用 `v0.9.0`。0.8.x 模型缓存和离线模型包无需重新下载或导出。
...
- 0.8.1 不需要清空模型缓存或重新导出 0.8.0 模型包。
...
0.9.0 本轮改动已在本地通过 77 项可执行的 Python 测试、34 项前端测试，以及标准版/兼容版的前端生产构建。本地副本未下载二进制图标资产且缺少 Chromium，因此仓库原有的图标资产校验和完整浏览器布局检查留给 Actions；Windows/Linux 原生安装包与模型推理检查也仍由 Actions 执行。这些检查均保留在工作流中。
...
- 重新启动不自动识别；上次正在运行/暂停的任务转回等待。0.9.0 起，设置和原文件未变化时可从已保存的下一未完成物理页继续；不恢复单页内部的原生推理。已保存的部分结果在重试前仍可导出。
...
- 页码与旋转设置随现有会话自动保存。旧会话默认全部页、0 度；0.9.0 新产生的部分 PDF 结果可从下一未完成物理页继续，0.8.x 中缺少文件指纹和阈值信息的旧部分结果会安全地从头重识别。
