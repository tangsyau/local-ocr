...
当前版本为 **0.8.1：Windows 初始化与小窗口预览修复版**。保留 0.8.0 的指定页码、图片旋转与离线模型迁移；沿用 0.7.5 的正式图标；没有新增快捷键，仅保留原有的专注模式 `Esc` 退出操作。
...
- 直接用此源码覆盖仓库对应文件再提交即可，保留自己的仓库设置；如用标签发布，请使用 `v0.8.1`。不需要清空模型缓存或重新导出 0.8.0 模型包。
...
此源码包已在本地通过 78 项 Python 测试、28 项前端测试，以及标准版/兼容版的前端生产构建。当前环境缺少 Chromium，完整浏览器布局检查未在本地完成；Windows/Linux 原生安装包与模型推理检查仍由 Actions 执行。这些检查均保留在工作流中。
